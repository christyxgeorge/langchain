...
class MessagesPlaceholder(BaseMessagePromptTemplate):
    ...
    def format_messages(self, **kwargs: Any) -> List[BaseMessage]:
        ...
        if not isinstance(value, list):
            raise ValueError(
                f"variable {self.variable_name} should be a list of base messages, " f"got {value}"
            )
        for v in value:
...
class ChatPromptTemplate(BaseChatPromptTemplate, ABC):
    ...
    def from_role_strings(cls, string_messages: List[Tuple[str, str]]) -> ChatPromptTemplate:
        ...
        """
        messages = [
            ChatMessagePromptTemplate(prompt=PromptTemplate.from_template(template), role=role)
